...
const page = () => {
  ...
  const searchName = param.get("name");
  ...
  //  validate : no gender data returned (API didn’t find the name)
  if (!userData.gender) {
    return (
      <div className="min-h-screen flex justify-center items-center font-work">
        <div className="bg-white rounded-2xl shadow-lg p-6 text-center max-w-sm">
          ...
          <p className="text-sm text-gray-600">
            No gender data available for this name.
          </p>
        </div>
      </div>
    );
  ...
  const cnfiProbability = Math.round(userData.probability * 100);
